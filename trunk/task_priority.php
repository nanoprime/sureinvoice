...
if($_REQUEST['mode'] == 'add'){
	...
	if(empty($_REQUEST['id'])){
		...
		$task_priority->get($_REQUEST['id']);
	}

	if($_POST['save']){
		$task_priority->updateFromAssocArray($_POST);
		if($task_priority->update()){
			goBack();
		}else{
			$error_msg .= "Error updating Task Priority!\n";
		}	
	}
}else if($_REQUEST['mode'] == 'delete'){
	$title = "Delete Task Priority";
	if($task_priority->delete($_REQUEST['id'])){
		goBack();
	}else{
		$error_msg .= "Error deleting Task Priority!\n";
	}
}else{
	$title = "Invalid Mode";
...
<tr>
	...
	<td class="form_field_cell"><input name="name" class="input_text" size="25" type="text" value="<?= $task_priority->name ?>"></td>
</tr>
<tr>
	<td class="form_field_header_cell">Priority Level</td>
	<td class="form_field_cell"><input name="priority_level" class="input_text" size="5" type="text" value="<?= $task_priority->priority_level ?>"></td>
</tr>
<tr>
