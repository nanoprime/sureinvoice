...
$title = 'Import Time - Map Item Codes';
...
if(count($importer->item_codes) <= 0){
	header("Location: ".getCurrentURL('time_import_6.php'));
	exit();	
}

if($_POST['save']){
	foreach ($_POST['actions'] as $normalizedName => $type){
		$importer->addItemCodeMapping($normalizedName, $type, $_POST['params'][$normalizedName]);
	}
	header("Location: ".getCurrentURL('time_import_6.php'));
	exit();
}
...
YAHOO.util.Event.addListener(window, 'load', getItemCodeList);
function getItemCodeList(){
	var handleSuccess = function(o){
		result = Uversa.SureInvoice.Timers.parseResponse(o);
		
		var list = document.getElementById('iclist');
		list.innerHTML = result;
	}
	YAHOO.util.Connect.asyncRequest('GET', 'json.php/importGetItemCodes', {success: handleSuccess});
}
</script>
<div class="box">
...
<tr>
	<td>
		<p>Below is a list of item codes from the file to be imported. If you would like to import the time associated with these tasks then you need to select Map as the Import Action and select a task from the list in the Map To column.</p>
	</td>
</tr>
<tr><td>
	<div class="tableContainer">
	<a href="javascript:;" class="tCollapse" onclick="toggleGrid(this)">
	<img src="images/arrow_down.jpg" alt="Hide table" />Item Codes</a><div>
	...
	<tbody id="iclist">
	<tr><td colspan="3"><img src="/images/spinner.gif" title="Loading Item Codes"> Loading Item Codes...</td></tr>
	</tbody>
	</table>
	</div></div>
</td></tr>
